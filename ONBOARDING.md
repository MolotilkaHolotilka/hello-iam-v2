...
### Сценарий B: «Сгенерируй контент и картинки с нуля» (медленный путь, fal.ai)

**Когда:** есть только бриф `content/posts/XXX_*.md`, ничего больше.

**Что сделать:**
1. Создай файл `apps/post-ops-ui/.env` (или скопируй `.env.example`), пропиши `FAL_KEY=...` (ключ с [fal.ai/dashboard](https://fal.ai/dashboard)).
2. Перезапусти `npm start`.
3. Открой UI → вкладка **Content V2** → выбери пост.
4. Пошагово пройди стадии: **Brief → Narrative → Slides → Prompts → Images → Export**. На каждой стадии нажимай «Generate» и «Approve».
5. Картинки сгенерируются через fal.ai и лягут в `content/artifacts/<postId>/`.

Дальше — обратно в Сценарий A: положить картинки в `public/generated/`, сделать шаблон, рендер.

**Что сказать Claude:** «Помоги дойти до изображений по посту 013 через Content V2». Он расскажет на каком ты шаге и что нажимать.

### Сценарий C: «Новый дизайн шаблона»
...
| **6 фоток слайдов** | сгенерил через fal.ai или своими руками | визуальная начинка карточек 2–6 (на квадратах 1024×645) |
...
| **бриф `.md` поста** | `content/posts/XXX_*.md` | если нужно сгенерить тексты (Сценарий B) |
...
- **`.env`** — никогда не коммить (там `FAL_KEY`).
...
| fal.ai возвращает 401/403 | `apps/post-ops-ui/.env` → `FAL_KEY` есть? Сервер перезапущен после правки? |
...
- Краткая операционная справка по UI и Content V2 — в [README.md](README.md).
