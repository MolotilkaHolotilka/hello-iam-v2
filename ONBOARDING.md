...
Открой [http://localhost:4242](http://localhost:4242) — это **Studio** (главная): выбор шаблона, редактор карточек, Run Render. Продвинутые сценарии — на [Import](http://localhost:4242/import.html) (JSON с диска, Import CSS). Если порт занят:
...
## 4. Studio, Import и CSS (Canvas MVP, путь A)

| Страница | URL | Когда |
|----------|-----|-------|
| **Studio** | `/` | Каждый пост: правка текстов, цветов, картинок по Card 1–7, сохранение на диск, Run Render |
| **Import** | `/import.html` | Редко: подложить `content.json` с диска; **новая серия** — вставить CSS из Figma и нажать Import CSS |
| **Assets** | `/assets.html` | Загрузка картинок/стикеров в папку шаблона |

**Два режима (см. [CANVAS_MVP_PLAN.md](docs/CANVAS_MVP_PLAN.md)):**
- **Новая серия** — один раз: CSS → Import → подправить в Studio (~10% руками).
- **Каждый пост** — только Studio: `content.json` (props), без правки `template.tsx`.

Сейчас в проде четыре deep-dive шаблона (lavash, dolma, matsun, khachkar) на общем `green-plate-intro`. Миграция на CSS-импорт — [MIGRATION_STEP7.md](docs/MIGRATION_STEP7.md); старые папки **не удаляем**, пока MP4 не сравнен.

API: `GET/PATCH /api/templates/:id/content`, `POST /api/templates/import-css` (нужен `OPENAI_API_KEY` в `.env` для live LLM).

---

## 5. Три сценария работы (как ты будешь это использовать)
...
## 7. Как выглядит контент-карточка изнутри (полезно понимать)
...
## 8. Видео внутри карточки (assetRole: "video")
...
## 9. Smoke-test (как убедиться что движок жив)
...
## 13. Стиль контента (важно для AI-генерации)
...
## 15. Как Claude этим всем пользуется (для понимания, что он видит)
...
- Canvas MVP (7 шагов) — [docs/CANVAS_MVP_PLAN.md](docs/CANVAS_MVP_PLAN.md).
- Миграция deep-dive → CSS import — [docs/MIGRATION_STEP7.md](docs/MIGRATION_STEP7.md).
